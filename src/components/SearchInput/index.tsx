...
const SearchInput = () => {
    ...
    const handleRedirect = (event: FormEvent) => {
        ...
        router.push(`/search/${filteredValue}`);
    };
    ...
    return (
        <div className="w-full max-w-sm">
            <form onSubmit={handleRedirect} className="flex items-center gap-2">
                ...
                <button
                    type="submit"
                    className="flex items-center h-12 border-2 border-transparent p-2 focus:border-2 focus:border-brand-400 outline-none rounded transition-colors hover:bg-neutral-800"
                >
                    <AiOutlineArrowRight className="fill-brand-400 h-8 w-8 transition-colors duration-200" />
                </button>
            </form>
        </div>
    );
